...
class FirstScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # 一级分屏1
        layPart1 = BoxLayout(orientation='horizontal', size_hint=(1, 0.1),pos_hint={"x":0,"y":0.9})
        # 二级分屏1.0
        ...
        grid_layout = GridLayout(cols=8, rows=13, spacing=1, padding=1)
        ...
        for row in range(0,13):
            for col in range(0,8):
                if row==0 and col==0:
                    button = Button(text=current_date.strftime('%m月%d日'), halign='center', valign='center',text_size=(window_width/8,window_height*0.9/13),font_size="15sp",font_name='chinese_font') 
                    ...
                    button = Button(text=str(config_col[row-1]), halign='center', valign='center',text_size=(window_width/8,window_height*0.9/13),font_size="15sp",font_name='chinese_font')  # 初始文本为空，设置字体大小
                    button.background_color = (1, 1, 1, 1)
                    grid_layout.add_widget(button)
                else:
                    button = Button(text=data[row-1][col-1],halign='center',  valign='center',text_size=(window_width/8,window_height*0.9/13), font_size="10sp",font_name='chinese_font',color=(0, 0, 0, 1),background_normal="")  # 初始文本为空，设置字体大小
                    button.background_color = (1, 1, 1, 1)
    ...
    def updata_button_text_according2week(self,week=1):
        ...
        for i in range(len(self.weekday_buttons)):
            ...
            self.weekday_buttons[i].background_color = (1, 1, 1, 1)
                    ...
                    if list_cache[5]!="None":
                        ...
                        for j in range(int(classTime_cache[0]),int(classTime_cache[1])+1):
                            if j==int(classTime_cache[0]):
                                ...
                                coun=chinese2num[list_cache[4]]-1+(int(j)-1)*7
                            self.buttons[coun].background_color = col_rbga
                            output_string = '\n'.join([list_cache[8],list_cache[10],list_cache[7]])
    ...
    def on_button_press(self, instance):
        ...
        # 创建关闭按钮
        close_button = Button(text='关闭', font_name="chinese_font", size_hint=(0.1, 0.1), pos_hint={"x": 0.9, "y": 0},background_color=(0, 0, 0, 0), on_press=lambda x: info_popup.dismiss())


        layout_label=BoxLayout(size_hint=(1,0.9))

        label = Label(text=additional_info[1], font_name="chinese_font",halign='left',size_hint=(1,1),font_size="20sp",text_size=(window_width*0.75,None),pos_hint={"x":0,"y":0})
        # 创建包含 Label 和关闭按钮的布局
