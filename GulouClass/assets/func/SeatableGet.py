from . import json, urllib_request, urlencode, URLError, ssl, getAuthorization
...
def SeatableGet(url,university,grade):
    try:
        try:
            ...
            response_code="Authorization获取失败"
        url_list = url.split('/')
        for i in url_list:
            if i=='':
                url_list.remove(i)
        ...
        result_data = [i for i in j['results']]
        # 读取导入的日期
        with open("assets/conf/cache_data.txt",'w', encoding='utf-8') as f:
            f.write(result_data[1]["r764"])
